---
name: "冲了吗 · 生物荧光双态记录仪"
description: "面向手机的私密自律记录界面，以昼夜双态的生命信号系统表达诚实、连续与恢复。"
colors:
  night-ink: "#061210"
  night-deep: "#030B0A"
  ...
  day-raised: "#F5FAF8"
  day-text: "#10241D"
  day-text-soft: "#29483D"
  day-muted: "#526D63"
  day-bio-mint: "#087A55"
  day-coral-alert: "#B83E38"
typography:
  display:
...
# Design System: 生物荧光双态记录仪

## Overview

**Creative North Star: “掌心里的生命信号舱”**

这是一个人在私人环境中快速完成记录的操作界面，不是宣传页，也不是道德审判台。夜间以深海墨绿承载冷青信号；日间以冷白偏青的纸面承载更深的薄荷信号。珊瑚红只承担“冲了”和错误状态，不作为装饰。页面始终先让用户完成今天的动作，再逐层看到统计、榜单、恢复与历史。
...
- 单列、拇指优先、首屏完成打卡。
- 跟随系统的昼夜双主题，薄荷绿为唯一品牌强调色；信息层级和状态语义在两套主题中完全一致。
...
夜间让冷青生命信号从偏蓝墨绿中浮出，日间则以深薄荷在冷白表面上保持可读性；红色始终严格绑定“冲了”与错误状态。
...
- **Day Canvas** (`#F3F8F6`): 白天模式全页背景，避免纯白眩光。
- **Day Surface** (`#FFFFFF`): 主要功能表面。
- **Day Raised** (`#F5FAF8`): 输入、分组与临时浮层。
- **Day Ink** (`#10241D`): 主文字与关键数字。
- **Day Muted** (`#526D63`): 次级说明；在白色和 Day Canvas 上均达到正文 AA 对比度。
- **Day Bio Mint** (`#087A55`): 白天模式状态绿，可作为正文与控件文字使用。
- **Day Coral** (`#B83E38`): 白天模式状态红，可作为正文与错误文字使用。
...
深度主要依靠色层、1px 主题色透明描边、内高光和局部光场，不使用纯黑重阴影。日间阴影必须带墨绿色调，夜间阴影融入深海背景；静止表面近乎平，选中、展开和拖动时才出现更明确的边缘与信号光场。
...
- 使用当前主题的实底表面、12px 圆角、最小高度 48px。
- 聚焦时边框与外环变成冷青；错误使用珊瑚红并显示文字，不只变色。

### Navigation

顶部只保留一个圆形更多按钮；页面不新增底栏或营销导航。固定“回到今天”按钮使用当前主题的高对比实底与薄荷细线，并避开底部安全区。
...
- **Do** 保留所有隐私与非医学说明，并使其在昼夜两套主题下都清晰可读。
