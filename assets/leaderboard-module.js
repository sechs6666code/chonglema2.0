...
(() => {
  ...
  function playBoardTransition(list, tone) {
    ...
    list.dataset.boardTransition = "entering";
    const accent = tone === "rush" ? "rgba(255, 116, 108, 0.24)" : "rgba(66, 245, 179, 0.22)";
    const animation = list.animate([
      { opacity: 0, transform: "translate3d(0, 12px, 0) scale(0.985)", filter: "blur(8px)", boxShadow: `0 0 0 ${accent}` },
      { opacity: 1, transform: "translate3d(0, 0, 0) scale(1)", filter: "blur(0)", boxShadow: `0 0 32px ${accent}` },
      { opacity: 1, transform: "translate3d(0, 0, 0) scale(1)", filter: "blur(0)", boxShadow: "0 0 0 transparent" },
    ], {
      duration: 420,
    ...
    animation.finished
      .catch(() => {})
      .finally(() => {
        if (boardAnimation === animation) {
          boardAnimation = null;
          delete list.dataset.boardTransition;
        }
        animation.cancel();
      });
  }
  ...
  function createDialog() {
      ...
      <section class="leaderboard-panel" role="dialog" aria-modal="true" aria-labelledby="leaderboard-title">
        ...
        <div class="leaderboard-scroll">
          ...
          <section class="leaderboard-board-card">
            <div class="leaderboard-board-toolbar">
    ...
    element.querySelectorAll("[data-tab]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        renderTabs();
        renderBoard();
        playBoardTransition(list, state.tab);
  ...
  function renderBoard() {
    ...
    const podium = entries.slice(0, 3).map((entry) => {
      ...
      const profileId = escapeHtml(`profile-${profileKey(entry.publicId)}`);
      return `<article class="leaderboard-podium-card rank-${rankNumber}${mine ? " is-mine" : ""}" data-profile-id="${profileId}" data-podium-rank="${rankNumber}">
    ...
    const rows = entries.slice(3).map((entry) => {
      const mine = profileKey(entry.publicId) === profileKey(state.profile.publicId);
      const profileId = escapeHtml(`profile-${profileKey(entry.publicId)}`);
      return `<div class="leaderboard-row${mine ? " is-mine" : ""}" data-profile-id="${profileId}">
